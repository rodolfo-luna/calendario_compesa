...
def main():
    url_abastecimento_atual = prepara_url(url_abastecimento, mes, 'abastecimento')
    df_calendario_abastecimento = extrai_calendario(url_abastecimento_atual)
    ...
    url_abastecimento_atual = prepara_url(url_manutencao, mes, 'manutencao')
...
def prepara_url(url, novo_mes, tipo_de_calendario):
    if tipo_de_calendario == 'abastecimento':
        inicio = re.search('CInicio\)%3D%27', url).span()[1]
        termino = re.search('Termino%20\)%3D%27', url_abastecimento).span()[1]
        url_final = url[:inicio] + novo_mes + url[inicio+2:termino] + novo_mes + url[termino+2:]
    elif tipo_de_calendario == 'manutencao':
        inicio = re.search('INICIO_PREVISTO\)%3D%27', url).span()[1]
        termino = re.search('TERMINO_PREVISTO%20\)%3D%27', url).span()[1]
        url_final = url[:inicio] + novo_mes + url[inicio+2:termino] + novo_mes + url[termino+2:]
    else:
        raise ValueError('Valor do tipo_de_calendario inválido, use abastecimento ou manutencao.')
